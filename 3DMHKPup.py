# Three-Dimensional Multiple Heterogeneous Knapsack Problem (3DMHKP) - "this
# way up" variant
# =========================================================================
...
# using the fractional knapsack problem", EJOR 74:143-151, with one practical
# constraint added: boxes may NOT be tipped onto a side.
#
...
#
# -------------------------------------------------------------------------
# WHAT'S DIFFERENT FROM 3DMHKP.py: THE ORIENTATION CONSTRAINT
# -------------------------------------------------------------------------
# Mohanty et al. and Bortfeldt both allow boxes to be freely rotated into any
# of up to 6 axis-aligned orientations. This file instead imposes the box
# orientation constraint of Bischoff & Ratcliff (1995) - "boxes with a 'this
# way up!' sign" - one of the 12 practical constraints in their survey, also
# implemented as a restricted rotation set Omega_i by Kurpel, Scarpin, Pecora,
# Schenekemberg & Coelho (2020, EJOR 284:87-107, Sec. 3).
#
...
#     (l, w, h) and (w, l, h)
# instead of all 6 permutations - 1 orientation instead of 2 if l == w. Every
# other constraint, the MILP structure, the greedy heuristic and the
# continuous-knapsack bound are unchanged from 3DMHKP.py: they are already
# generic over whatever orientation list a box carries.
#
# -------------------------------------------------------------------------
# BOX VALUES
# -------------------------------------------------------------------------
...
#       v_i = c_i * l_i * w_i * h_i
#
# This is verified in 3DMHKP.py: with this convention the continuous-knapsack
# relaxation reproduces Bortfeldt's published upper bounds in Table 2 EXACTLY
# for all 16 instances. Use --value-mode flat to treat c_i as an absolute
# per-box value instead.
#
# continuous_knapsack_bound() only sums box volumes and values, never their
# orientations, so it is numerically IDENTICAL here to the unrestricted
# problem's bound - it is still a valid upper bound (it over-counts what a
# 1-D relaxation can never see), but the "this way up" restriction can only
# ever make the true optimum lower, so this bound is now looser than it was
# in 3DMHKP.py and the reported gap correspondingly larger.
#
# -------------------------------------------------------------------------
# THE MILP
# -------------------------------------------------------------------------
# Box types are expanded into individual boxes j in J, container types into
# individual containers k in K. Each box has only its 1-2 upright orientations
# (see above), NOT the up to 6 of the unrestricted problem.
#
#   Variables
...
# USAGE
# -------------------------------------------------------------------------
#   python 3DMHKPup.py                          # all 16, 300 s each
#   python 3DMHKPup.py --instances 7             # a single instance
#   python 3DMHKPup.py --instances 1 7 9 --time-limit 60
#   python 3DMHKPup.py --bound-only              # LP/knapsack bounds, no MILP
#   python 3DMHKPup.py --greedy-only             # heuristic only, no MILP
...
def build_model(inst, compat, warm_start=None, verbose=False):
    ...
    model = gp.Model(f"3DMHKPup_{inst['name']}")
...
def _finish(inst, placement, result, args):
    ...
    if not args.no_report:
        RESULTS_DIR.mkdir(parents=True, exist_ok=True)
        out = RESULTS_DIR / f"{inst['name']}-3DMHKPup.txt"
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Exact MILP for the 3D multiple heterogeneous knapsack "
                    "problem of Mohanty, Mathur & Ivancic (1994), with boxes "
                    "restricted to their upright ('this way up') orientations.")
