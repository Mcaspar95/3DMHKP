# Simulated Annealing for the 3DMHKP — "this way up" sequence-decoder variant
# =========================================================================
# Heuristic counterpart to the exact MILP in 3DMHKPup.py for the packing
...
# knapsack problem", EJOR 74:143-151, with one practical constraint added:
# boxes may NOT be tipped onto a side.
#
...
#   of the stowed cargo is maximal. Boxes keep their height upright and may
#   only be turned about the vertical axis (1-2 orientations instead of the
#   unrestricted problem's 6), box values are v_i = c_i * l_i*w_i*h_i.
#
# -------------------------------------------------------------------------
# WHAT'S DIFFERENT FROM 3DMHKP-SA.py: THE ORIENTATION CONSTRAINT
# -------------------------------------------------------------------------
# This file is 3DMHKP-SA.py under the box orientation constraint of Bischoff &
# Ratcliff (1995) - "boxes with a 'this way up!' sign" - one of the 12 practical
# constraints in their survey, also implemented as a restricted rotation set
# Omega_i by Kurpel, Scarpin, Pecora, Schenekemberg & Coelho (2020, EJOR
# 284:87-107, Sec. 3), exactly as 3DMHKPup.py imposes it on the exact MILP.
#
# A box's height h is fixed to always point up: the box may only be rotated
# about the VERTICAL axis, swapping its footprint between (l, w) and (w, l).
# Concretely, unique_orientations(l, w, h) below returns just
#     (l, w, h) and (w, l, h)
# instead of all 6 permutations - 1 orientation instead of 2 if l == w. The SA
# engine, the decoder, the move set and the bound are unchanged: they are
# already generic over whatever orientation list a box carries. The one
# behavioural consequence is that the `orient` operator now merely flips a
# footprint (and is a no-op for square-footprint box types, which the sampler
# already skips), so the search leans more on the priority-order operators.
#
...
# That is the same construction as greedy_pack() in 3DMHKPup.py, but driven by
...
#   orient    — change the orientation preference of one box (here: flip its
#               footprint between (l, w) and (w, l); h always stays vertical)
...
# USAGE
# -------------------------------------------------------------------------
#   python 3DMHKPup-SA.py                            # all 16, 300 s each
#   python 3DMHKPup-SA.py --instances 7              # a single instance
#   python 3DMHKPup-SA.py --instances 1 7 9 --time-limit 60
#   python 3DMHKPup-SA.py --alpha 0.9995 --seed 1    # SA parameter variants
#   python 3DMHKPup-SA.py --greedy-only              # decode the initial order only
...
# =========================================================================
# Instance parsing  (identical format and value model to 3DMHKPup.py)
# =========================================================================
...
def unique_orientations(l, w, h):
    """The orientations allowed for a box marked THIS WAY UP.

    Only rotation about the vertical axis is allowed: h always stays vertical,
    but the horizontal footprint may be placed as (l, w) or (w, l). 2 distinct
    orientations, or 1 if l == w (a square footprint has nothing to rotate
    into). See the module header for the practical-constraint rationale.
    """
    return list(dict.fromkeys([(l, w, h), (w, l, h)]))
...
def build_instance(path, value_mode="volume"):
    ...
    reproduces Bortfeldt's (2000) published upper bounds exactly. It is
    independent of the orientation constraint, so it carries over unchanged.
    """
...
def continuous_knapsack_bound(inst):
    ...
    It only sums box volumes and values, never their orientations, so it is
    numerically IDENTICAL to the unrestricted problem's bound. It stays a valid
    upper bound - "this way up" can only ever make the true optimum lower - but
    it is correspondingly looser here, and the reported gaps larger.
    """
...
# =========================================================================
# Reporting  (same report layout as 3DMHKPup.py, plus the SA block)
# =========================================================================
...
def write_report(path, inst, placement, result):
    ...
    with open(path, "w") as f:
        ...
        f.write("Orientation constraint: THIS WAY UP - boxes may only rotate "
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Simulated annealing for the 3D multiple heterogeneous "
                    "knapsack problem of Mohanty, Mathur & Ivancic (1994), with "
                    "boxes restricted to their upright ('this way up') "
                    "orientations.")
